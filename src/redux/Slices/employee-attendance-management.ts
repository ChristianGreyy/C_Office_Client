...
import { getAllEmployeeAttendancesAction, getEmployeeAttendanceByIdAction, updateEmployeeAttendanceByIdAction } from '../actions'
...
const employeeAttendancesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(updateEmployeeAttendanceByIdAction.pending, (state) => {
      state.loadings[`updateEmployeeAttendanceByIdActionLoading`] = true
    })
    builder.addCase(updateEmployeeAttendanceByIdAction.fulfilled, (state, action) => {
      state.loadings[`updateEmployeeAttendanceByIdActionLoading`] = false
      state.selectedEmployeeAttendance = action.payload
    })
    builder.addCase(updateEmployeeAttendanceByIdAction.rejected, (state) => {
      state.loadings[`updateEmployeeAttendanceByIdActionLoading`] = false
    })
  },
})
...
// export const selectEmployeeAttendancesLoading = (state: RootState, name: string) =>
